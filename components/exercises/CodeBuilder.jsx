...
const BLANKS = [
  {
    ...
    explanation: (
      <>
        Right! <code>+=</code> keeps adding onto our running total, and{" "}
        <code>arr[i]</code> is the number at the loop&apos;s current position -
        not <code>arr[k]</code>, which would grab the same one number every
        single round.
      </>
    ),
  },
  {
    ...
    explanation: (
      <>
        Exactly - before we&apos;ve compared anything else, our only window{" "}
        <em>is</em> the best one we&apos;ve seen, so <code>bestSum</code> starts
        out equal to <code>windowSum</code>.
      </>
    ),
  },
  {
    ...
    explanation: (
      <>
        <code>start</code> is the index that is leaving the window, so we
        subtract <code>arr[start]</code> - that one subtraction is the whole
        trick that makes sliding window fast!
      </>
    ),
  },
  {
    ...
    explanation: (
      <>
        We add <code>arr[end]</code> because that&apos;s the new number that
        just entered the window on the right edge.
      </>
    ),
  },
  {
    ...
    explanation: (
      <>
        <code>Math.max</code> keeps whichever of the two is bigger - so{" "}
        <code>bestSum</code> only changes when we&apos;ve genuinely found a new
        champion window.
      </>
    ),
  },
        ...
        <div>
          ...
          <p className="text-sm text-ink-500">
            {finished
              ...
              : `Line ${step + 1} of ${BLANKS.length} - let's build it together.`}
          </p>
        </div>
      </div>
      ...
      {!finished ? (
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {wrongPick && !solved && (
              <p className="mt-2 text-xs font-medium text-terracotta-600">
        ...
        >
          🌟 Every line above is exactly the real <code>maxSumSubarray</code>{" "}
          function - and you chose each meaningful line yourself. Try pasting it
          into your browser&apos;s console; it really runs!
        </motion.div>
      )}
    </div>
  );
