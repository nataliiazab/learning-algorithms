  ...
  return (
    <div className="rounded-3xl border border-lavender-200 bg-white/80 p-6 sm:p-8">
      <div className="mb-6 flex items-start gap-3">
        ...
        <div>
          <h3 className="font-heading text-lg text-ink-900">{walkthrough.title}</h3>
          {walkthrough.intro && <p className="mt-1 text-sm leading-relaxed text-ink-700">{walkthrough.intro}</p>}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-8">
        {walkthrough.parts.map((part, i) => (
          <div key={i} className="border-l-4 border-lavender-200 pl-4 sm:pl-6">
            <h4 className="mb-2 font-heading text-base text-ink-900">{part.heading}</h4>
      ...
      {walkthrough.closing && (
        <div className="mt-6 flex items-start gap-3 rounded-2xl bg-sage-50 p-4">
          ...
          <p className="text-sm leading-relaxed text-sage-800">{walkthrough.closing}</p>
        </div>
      )}
    </div>
  );
