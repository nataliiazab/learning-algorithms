...
function StatCard({ label, value, tone, icon }) {
  ...
  return (
    <div
      className={`flex flex-1 flex-col items-center gap-0.5 rounded-2xl border-2 px-4 py-3 ${tones[tone]}`}
    >
...
export default function WindowVisualizer({ frame }) {
  ...
  const showBestOutline =
    hasBest &&
    !finished &&
    (bestStart !== windowStart || bestEnd !== windowEnd);
  ...
  const cellStyles = {
    ...
    inWindow: finished
      ...
      : "bg-honey-200 text-ink-900 border-honey-300",
        ...
        >
          ...
          <div className="relative z-10 mt-1.5 flex gap-2.5">
            {array.map((_, index) => (
              <div
                key={index}
                className="flex h-4 w-14 shrink-0 items-center justify-center text-[10px] text-ink-300"
              >
                {index}
              </div>
            ))}
