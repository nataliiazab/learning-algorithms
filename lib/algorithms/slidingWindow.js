  ...
  for (let start = 1; start + size - 1 < n; start++) {
    ...
    steps.push(
      snap({
        ...
        loop: "slide",
        vars: { start: start - 1, end, "songScores[start]": leaving, runScore: sum },
...
export const slidingWindow = {
  ...
  walkthrough: {
    ...
    parts: [
      ...
      },
      {
        ...
        table: {
          columns: ['Round', 'i', 'Is i < 3?', 'What happens', 'runScore after'],
          rows: [
            ['1', '0', '0 < 3 → yes', 'runScore += songScores[0] = 2', '0 + 2 = 2'],
            ['2', '1', '1 < 3 → yes', 'runScore += songScores[1] = 1', '2 + 1 = 3'],
            ['3', '2', '2 < 3 → yes', 'runScore += songScores[2] = 5', '3 + 5 = 8'],
      ...
      },
      {
        ...
        table: {
          columns: ['Round', 'end', 'start (end − 3)', 'Leaving: songScores[start]', 'Entering: songScores[end]', 'runScore: before → after', 'New best?'],
          rows: [
            ['1', '3', '0', '2', '1', '8 − 2 + 1 = 7', '7 < 8 → no'],
            ['2', '4', '1', '1', '3', '7 − 1 + 3 = 9', '9 > 8 → YES, bestRunScore = 9'],
      ...
      },
      {
        ...
        table: {
          ...
          rows: [
            ['songScores', 'the whole function (a parameter)', 'The actual list of numbers you handed the function', "Never — parameters just get read, they don't change"],
            ['songsPerRun', 'the whole function (a parameter)', 'The run size, chosen by whoever calls the function', 'Never — stays fixed for the whole function call'],
            ['runScore', 'both loops', 'Tracks the score of whatever songs are in the run RIGHT NOW', 'Every single round'],
            ['bestRunScore', 'set after loop 1, updated in loop 2', 'Remembers the best runScore ever seen', 'Only when runScore beats it'],
            ['i', 'loop 1 only', 'Walks through positions 0..songsPerRun-1 to build the first run', 'Every round of loop 1, then gone for good'],
            ['end', 'loop 2 only', 'Marks the new right edge sliding into the run', 'Every round of loop 2'],
            ['start', 'loop 2 only', 'Marks the old left edge sliding out of the run', 'Automatically, since start = end − songsPerRun'],
          ],
        },
      },
    ],
  ...
  quiz: [
    ...
    },
    {
      ...
      options: ["Just 1", "Exactly songsPerRun", "n minus 1", "Infinitely many"],
      correctIndex: 0,
      explanation:
  ...
  translations: {
    uk: {
      ...
      walkthrough: {
        ...
        parts: [
          {
            heading: 'Частина 1 — Зачекай, що взагалі таке songScores і songsPerRun?',
            paragraphs: [
          ...
          },
          {
            ...
            table: {
              columns: ['Раунд', 'i', 'Чи i < 3?', 'Що відбувається', 'runScore після'],
              rows: [
                ['1', '0', '0 < 3 → так', 'runScore += songScores[0] = 2', '0 + 2 = 2'],
                ['2', '1', '1 < 3 → так', 'runScore += songScores[1] = 1', '2 + 1 = 3'],
                ['3', '2', '2 < 3 → так', 'runScore += songScores[2] = 5', '3 + 5 = 8'],
                ['—', '3', '3 < 3 → НІ, стоп', '(цикл зупиняється тут)', 'лишається 8'],
              ],
            },
          ...
          },
          {
            ...
            table: {
              columns: ['Раунд', 'end', 'start (end − 3)', 'Виходить: songScores[start]', 'Заходить: songScores[end]', 'runScore: до → після', 'Новий рекорд?'],
              rows: [
                ['1', '3', '0', '2', '1', '8 − 2 + 1 = 7', '7 < 8 → ні'],
                ['2', '4', '1', '1', '3', '7 − 1 + 3 = 9', '9 > 8 → ТАК, bestRunScore = 9'],
          ...
          },
          {
            ...
            table: {
              columns: ['Змінна', 'Живе в', 'Навіщо існує', 'Змінюється, коли...'],
              rows: [
                ['songScores', 'уся функція (параметр)', 'Список чисел, який отримала функція', 'Ніколи - параметри просто читаються, а не змінюються'],
                ['songsPerRun', 'уся функція (параметр)', 'Розмір прогону, обраний тим, хто викликає функцію', 'Ніколи - лишається незмінним протягом усього виклику'],
                ['runScore', 'обох циклах', 'Відстежує суму пісень, що ЗАРАЗ у прогоні', 'Кожного раунду'],
                ['bestRunScore', 'встановлюється після циклу 1, оновлюється у циклі 2', "Пам'ятає найкращу runScore з усіх", 'Лише коли runScore її перевершує'],
                ['i', 'лише у циклі 1', 'Проходить позиції 0..songsPerRun-1, будуючи перший прогін', 'Кожного раунду циклу 1, потім зникає назавжди'],
                ['end', 'лише у циклі 2', 'Позначає нову пісню, що заходить у прогін справа', 'Кожного раунду циклу 2'],
                ['start', 'лише у циклі 2', 'Позначає стару пісню, що виходить з прогону зліва', 'Автоматично, бо start = end − songsPerRun'],
              ],
            },
          },
        ],
      ...
      conceptGame: [
        ...
        },
        {
          ...
          isCode: true,
          answer: "loop",
          feedback: "Правильно - цикл Будівельник, що повторюється songsPerRun разів.",
        },
        {
          term: "bestRunScore",
          isCode: false,
      ...
      quiz: [
        ...
        },
        {
          ...
          options: ["Лише 1", "Рівно songsPerRun", "n мінус 1", "Нескінченно багато"],
          correctIndex: 0,
          explanation:
